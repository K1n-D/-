...
Start-Process python -ArgumentList '-m','iot_middleware.broker' -WorkingDirectory $root -RedirectStandardOutput "$root\logs\mqtt.log" -RedirectStandardError "$root\logs\mqtt-error.log" -WindowStyle Hidden | Out-Null
for ($i = 0; $i -lt 20; $i++) {
