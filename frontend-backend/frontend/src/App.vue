...
const histDevice = ref(''), histPoint = ref(''), histBusy = ref(false)
...
async function loadHistoryChart() {
  ...
  histBusy.value = true
  try {
    const rows: Telemetry[] = await request(`/api/history?device=${encodeURIComponent(histDevice.value)}&point=${encodeURIComponent(histPoint.value)}&limit=300`)
    await nextTick()
    if (chartEl.value) {
