...
const streamChartEl = ref<HTMLDivElement>()
const historyChart = ref<ReturnType<typeof echarts.init> | undefined>(undefined)
const streamChart = ref<ReturnType<typeof echarts.init> | undefined>(undefined)
...
const onlinePct = computed(() => stats.value.deviceTotal ? Math.round(stats.value.online / stats.value.deviceTotal * 100) : 0)
const pulseRingStyle = computed(() => ({
  background: `radial-gradient(circle, #fff 57%, transparent 58%), conic-gradient(#54aa91 0 ${onlinePct.value}%, #e7efed ${onlinePct.value}% 100%)`
}))
...
// Stable option lists for the history page: derived from the full device
// inventory plus the point table, never from the sliding telemetry window
// (whose membership/order changes every second as new readings arrive).
const historyDeviceOptions = computed(() => {
  const set = new Set<string>()
  devices.value.forEach(d => set.add(d.deviceCode))
  pointRows.value.forEach(p => set.add(p.deviceCode))
  if (histDevice.value) set.add(histDevice.value)
  return [...set].sort()
})
const historyPointOptions = computed(() => {
  const set = new Set<string>()
  pointRows.value.filter(p => !histDevice.value || p.deviceCode === histDevice.value)
    .forEach(p => set.add(p.pointCode))
  devices.value.filter(d => !histDevice.value || d.deviceCode === histDevice.value)
    .forEach(d => Object.keys(d.points || {}).forEach(k => set.add(k)))
  if (histPoint.value) set.add(histPoint.value) // keep the current selection selectable
  return [...set].sort()
})
const sortedTelemetry = computed(() => telemetry.value.slice().sort((a, b) => +new Date(b.timestamp) - +new Date(a.timestamp)))
// The stream chart follows ONE stable device/point (auto-picked, switchable)
// — chasing the latest reading would jump between devices with different
// baselines and read as wild swings.
const streamFocus = ref('')  // "DEVICE|point"; empty = auto-pick the first pair
const streamPairs = computed(() => {
  const pairs = new Set<string>()
  devices.value.forEach(d => Object.keys(d.points || {}).forEach(p => pairs.add(`${d.deviceCode}|${p}`)))
  pointRows.value.forEach(p => pairs.add(`${p.deviceCode}|${p.pointCode}`))
  return [...pairs].sort()
})
const streamFocusKey = computed(() => {
  if (streamFocus.value && streamPairs.value.includes(streamFocus.value)) return streamFocus.value
  return streamPairs.value[0] || null
})
const focusSeries = computed(() => {
  const key = streamFocusKey.value
  if (!key) return null
  const sep = key.indexOf("|")
  const device = key.slice(0, sep), point = key.slice(sep + 1)
  const head = telemetry.value.find(row => row.deviceCode === device && row.pointCode === point)
  const unit = head?.unit || ''
  const points = telemetry.value
    .filter(row => row.deviceCode === device && row.pointCode === point && typeof row.value === 'number')
    .sort((a, b) => +new Date(a.timestamp) - +new Date(b.timestamp))
    .slice(-40)
    .map(row => ({ time: formatTime(row.timestamp), value: Number(row.value) }))
  return { device, point, unit, points }
})
const AREA_GRADIENT = {
  type: 'linear', x: 0, y: 0, x2: 0, y2: 1,
  colorStops: [{ offset: 0, color: 'rgba(63,157,136,.30)' }, { offset: 1, color: 'rgba(63,157,136,.02)' }]
}
// Padded Y bounds so normal process fluctuation reads as calm instead of
// filling the whole plot height.
const PADDED_Y = {
  min: (v: { min: number; max: number }) => v.min - (v.max - v.min) * 0.25 - 0.001,
  max: (v: { min: number; max: number }) => v.max + (v.max - v.min) * 0.25 + 0.001
}
const streamOption = computed(() => {
  ...
  return {
    ...
    yAxis: { type: 'value', min: PADDED_Y.min, max: PADDED_Y.max, splitNumber: 4, splitLine: { lineStyle: { color: '#eef3f3' } }, axisLabel: { color: '#93a3a6', fontSize: 9, fontFamily: 'DM Mono, monospace' } },
    series: [{
...
function renderStreamChart() {
  if (!streamChartEl.value) return
  streamChart.value = streamChart.value || echarts.init(streamChartEl.value)
  if (!streamOption.value) {
    streamChart.value.clear()
    return
  }
  streamChart.value.setOption(streamOption.value, true)
}
watch(streamOption, () => { if (currentView.value === 'dashboard') nextTick(renderStreamChart) })
...
async function loadHistoryChart() {
  ...
  try {
    ...
    await nextTick()
    if (chartEl.value) {
      historyChart.value = historyChart.value || echarts.init(chartEl.value)
...
watch(histDevice, () => { histPoint.value = '' })
watch(currentView, view => {
  if (view === 'config') loadPoints()
  if (view === 'alarms') loadRules()
  if (view === 'dashboard') nextTick(renderStreamChart)
})
function handleResize() { streamChart.value?.resize(); historyChart.value?.resize() }
onMounted(() => { if (token.value) { load(); connectStream() } ; timer = window.setInterval(load, 3000); window.addEventListener('resize', handleResize) }); onUnmounted(() => { if (timer) window.clearInterval(timer); window.removeEventListener('resize', handleResize); stream?.close(); historyChart.value?.dispose(); streamChart.value?.dispose() })
</script>
