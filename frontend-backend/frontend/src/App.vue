...
function renderStreamChart() {
  ...
  streamChart.value.setOption({
    ...
    series: [{
      ...
      lineStyle: { color: '#3f9d88', width: 2 },
      areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(63,157,136,.30)' }, { offset: 1, color: 'rgba(63,157,136,.02)' }]) }
    }]
  }, true)
}
watch(focusSeries, () => { if (currentView.value === 'dashboard') nextTick(renderStreamChart) })
...
async function savePoint(row: any) {
  try {
    await request(`/api/points/${encodeURIComponent(row.deviceCode)}/${encodeURIComponent(row.pointCode)}`, {
      ...
      body: JSON.stringify({ scaleFactor: Number(row.scaleFactor), deadZone: Number(row.deadZone), collectIntervalMs: Number(row.collectIntervalMs), unit: row.unit, enabled: !!row.enabled })
    })
    error.value = ''
    await loadPoints()
    ...
    if (chartEl.value) {
      ...
      historyChart.value.setOption({
        ...
        series: [{
          ...
          lineStyle: { color: '#3f9d88', width: 2 },
          areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(63,157,136,.28)' }, { offset: 1, color: 'rgba(63,157,136,.02)' }]) }
        }]
      }, true)
    }
    error.value = ''
...
watch(currentView, view => {
  ...
  if (view === 'history') nextTick(() => historyChart.value?.resize())
