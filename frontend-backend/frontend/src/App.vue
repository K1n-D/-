...
let historyChart: echarts.ECharts | undefined, stream: EventSource | undefined
...
const chart = computed(() => { const rows = telemetry.value.filter(row => typeof row.value === 'number').slice(0, 18).reverse(); if (!rows.length) return { bars: [] as number[], start: '', end: '' }; const values = rows.map(row => Number(row.value)); const min = Math.min(...values), max = Math.max(...values); return { bars: values.map(value => Math.round(24 + ((value - min) / Math.max(max - min, 1)) * 65)), start: formatTime(rows[0].timestamp), end: formatTime(rows[rows.length - 1].timestamp) } })
...
async function load() {
  ...
  try {
    ...
    telemetry.value = Array.isArray(t) ? t.slice(-120).reverse() : []
...
async function loadHistoryChart() {
  ...
  try {
    ...
    await nextTick()
    if (chartEl.value) {
      historyChart = historyChart || echarts.init(chartEl.value)
      const ordered = rows.slice().reverse().filter(row => typeof row.value === 'number')
      chart.setOption({
        grid: { left: 46, right: 16, top: 18, bottom: 26 },
        xAxis: { type: 'category', data: ordered.map(row => formatTime(row.timestamp)) },
        yAxis: { type: 'value', scale: true },
        tooltip: { trigger: 'axis' },
        series: [{ type: 'line', data: ordered.map(row => Number(row.value)), smooth: true, showSymbol: false, lineStyle: { color: '#3f9d88', width: 2 }, areaStyle: { color: 'rgba(63,157,136,.12)' } }]
      }, true)
    }
    error.value = ''
...
watch(currentView, view => {
  ...
  if (view === 'history') nextTick(() => historyChart?.resize())
})
onMounted(() => { if (token.value) { load(); connectStream() } ; timer = window.setInterval(load, 3000) }); onUnmounted(() => { if (timer) window.clearInterval(timer); stream?.close(); historyChart?.dispose() })
</script>
